...
export const logout = async (token) => {
    ...
    // Also remove the device account if userId is available
    try {
        ...
        if (userId) {
            ...
            if (ok) {
                console.log('Device account removed on logout');
            }
        }
    } catch (error) {
        console.warn('Remove device account failed:', error?.message);
    }
}
