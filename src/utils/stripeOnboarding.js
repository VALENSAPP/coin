...
export async function openStripeOnboarding(options = {}) {
  ...
  try {
    ...
    const message = error?.response?.data?.message ?? error?.message ?? 'Failed to open Stripe setup';
...
export async function openCustomerSetup(options = {}) {
  ...
  try {
    ...
    if (!url) throw new Error(STRIPE_ERROR_MESSAGES.CUSTOMER_SETUP_FAILED);
    if (await InAppBrowser.isAvailable()) {
      await InAppBrowser.open(url, STRIPE_BROWSER_OPTIONS);
    } else {
      await Linking.openURL(url);
    ...
    const message = error?.response?.data?.message ?? error?.message ?? STRIPE_ERROR_MESSAGES.CUSTOMER_SETUP_FAILED;
    throw new Error(message);
  }
}
