...
const MyClosetBuyerItemsScreen = ({ navigation, route }) => {
  ...
  const [loggedInUserId, setLoggedInUserId] = useState(route?.params?.loggedInUserId || null);
  const accent = text;
  ...
  const openItem = useCallback(
    ...
    [items, navigation, route, seller, sellerId, battleWinnerMap, loggedInUserId],
  );
...
const MyClosetBuyerReviewScreen = ({ navigation, route }) => {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (!checkoutUrl) {
        Alert.alert(t('myClosetBuyer.errorTitle'), response.message || t('myClosetBuyer.checkoutError'));
        return;
      }
