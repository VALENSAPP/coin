...
const MyClosetEarningsScreen = ({ navigation }) => {
  const { t } = useLanguage();
  const { bgStyle, textStyle, cardStyle, text, accent, mutedText, mutedTextStyle, border } = useAppTheme();
  const { isDarkMode } = useThemeContext();
  const chipSurface = isDarkMode ? 'rgba(255,255,255,0.08)' : 'transparent';
  ...
  return (
    <ScrollView style={[styles.container, bgStyle]} contentContainerStyle={styles.content}>
      <View style={[styles.header, { borderBottomColor: border }]}>
        <TouchableOpacity style={[styles.backButton, { backgroundColor: chipSurface }]} onPress={handleBack}>
      ...
      {loading ? (
        ...
        <>
          <View style={[styles.card, cardStyle, styles.earningsCard, { borderColor: border }]}>
            <Text style={[styles.sectionLabel, mutedTextStyle]}>{t('myClosetEarnings.totalEarningsLabel') || 'Total earnings'}</Text>
            ...
            <View style={styles.breakdownList}>
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownLabel, mutedTextStyle]}>{t('myClosetEarnings.itemSales') || 'Item sales'}</Text>
                <Text style={[styles.breakdownValue, textStyle]}>{itemSales}</Text>
              </View>
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownLabel, mutedTextStyle]}>{t('myClosetEarnings.shippingEarnings') || 'Shipping earnings'}</Text>
                <Text style={[styles.breakdownValue, textStyle]}>{shippingEarnings}</Text>
              </View>
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownLabel, mutedTextStyle]}>{t('myClosetEarnings.fees') || 'Fees'}</Text>
                <Text style={[styles.breakdownValue, textStyle]}>{fees.startsWith('-') ? fees : `-${fees}`}</Text>
              </View>
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownLabel, mutedTextStyle]}>{t('myClosetEarnings.payouts') || 'Payouts'}</Text>
                <Text style={[styles.breakdownValue, textStyle]}>{payouts.startsWith('-') ? payouts : `-${payouts}`}</Text>
              </View>
            </View>

            <View style={styles.balanceRow}>
              <Text style={[styles.balanceLabel, mutedTextStyle]}>{t('myClosetEarnings.availableBalance') || 'Available balance'}</Text>
              <Text style={[styles.balanceValue, textStyle]}>{availableBalance}</Text>
            </View>

            <TouchableOpacity style={[styles.withdrawButton, { backgroundColor: accent }]} activeOpacity={0.8} onPress={handleWithdraw}>
              <Text style={styles.withdrawButtonText}>{t('myClosetEarnings.withdrawButton') || 'Withdraw'}</Text>
            </TouchableOpacity>
          </View>

          <View style={[styles.card, cardStyle, { borderColor: border }]}>
            <Text style={[styles.sectionTitle, textStyle]}>{t('myClosetEarnings.payoutHistoryTitle') || 'Payout history'}</Text>
            {history.length > 0 ? (
              history.map(item => (
                <View key={item.id} style={[styles.historyRow, { borderBottomColor: border }]}>
                  <View style={styles.historyLeft}>
                    <Text style={[styles.historyDate, textStyle]}>
                      {item.orderNumber || item.buyerName}
                    </Text>
                    <Text style={[styles.historyMeta, mutedTextStyle]}>
              ...
              <Text style={[styles.emptyText, mutedTextStyle]}>{t('myClosetEarnings.noHistory') || 'No payout history available.'}</Text>
            )}
          </View>
        </>
      )}
