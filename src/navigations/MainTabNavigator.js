  ...
  export default function MainTabNavigator() {
    ...
    const { bgStyle, textStyle, bg, text, card, border, mutedText, accent, icon } = useAppTheme(profileThemeType);
    const { isDarkMode } = useThemeContext();
    const headerTitleColor = isDarkMode ? '#ffffff' : '#111827';
    const headerMenuColor = profile === 'company' ? text : (isDarkMode ? icon : text);
    ...
    // ── TRANSLATION CHANGE: t added to dependency array so stack rebuilds on lang change ──
    }, [t, bg, text, icon, userProfile, headerTitleColor, headerMenuColor, profile, isDarkMode]);
    ...
    const getScreenOptions = useCallback(
      ({ route, navigation }) => {
        ...
        const baseOptions = {
          ...
          tabBarActiveTintColor: route.name === 'Reels' && isFocused ? '#ffffff' : accent,
          tabBarInactiveTintColor: route.name === 'Reels' && isFocused ? '#ffffff' : mutedText,
          tabBarHideOnKeyboard: true,
          tabBarIcon: ({ focused, color, size }) => {
            const isReelsFocused = route.name === 'Reels' && isFocused;
            const iconColor = isReelsFocused ? '#ffffff' : (focused ? accent : mutedText);
